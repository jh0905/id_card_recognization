...
"""
 ...
 @time: 6/18/19 8:58 PM
 @desc: 将draft代码中的步骤,按照流程封装在函数里面,逻辑更为清晰,在本地实现身份证号码的提取功能
"""
import cv2
import numpy as np
import pytesseract
...
def main():
    ...
    if len(number_region) == 0:
        image_resize, image_preprocessed = img_preprocess(picture_path, [0, 1, 0])
        number_region = find_number_region(image_preprocessed)

    # step 3:get id number image
    image_id_number = get_number_img(image_resize, number_region)

    # step 4:horizontal correct the number image
