...
# ******************* flask服务器端基本配置 ***************************
ALLOWED_EXTENSIONS = {'png', 'jpg', 'jpeg', 'peg'}
...
def img_preprocess(pic_path, coefs):
    ...
    if img.shape[0] > img.shape[1]:
        img = np.rot90(img)
    if img.shape[0] > 2000:
        img_resize = cv2.resize(img, (img.shape[1] // 9, img.shape[0] // 9), interpolation=cv2.INTER_CUBIC)
    elif img.shape[0] > 1000:
        img_resize = cv2.resize(img, (img.shape[1] // 3, img.shape[0] // 3), interpolation=cv2.INTER_CUBIC)
    else:
        img_resize = cv2.resize(img, (428, 270), interpolation=cv2.INTER_CUBIC)
...
def upload_file():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            file_path = os.path.join(app.config['UPLOAD_FOLDER'], 'id_card_number.jpg')
            file.save(file_path)
            # 保存完图片之后，开始进行身份证号码提取
            try:
            ...
            return json.dumps({'code': -1, 'card_number': '-1', 'info': 'Input File Occurs Error!'})
